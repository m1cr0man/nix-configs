...
in
{
  ...
  systemd.services.gamesvm = {
    ...
    script = ''
      		...
      		${pkgs.qemu}/bin/qemu-system-x86_64 \
      		  ...
      		  -cpu host,hv_passthrough,kvm=off,-vmx  \
                        ...
                        -nodefaults \
      		  -vga qxl \
      		  -spice ipv4,tls-port=5910,addr=0.0.0.0,password="$SPICE_PASSWORD",tls-ciphers=HIGH,x509-key-file=${cert}/key.pem,x509-cert-file=${cert}/cert.pem,x509-cacert-file=${cert}/chain.pem \
    ...
    serviceConfig = {
      ...
      EnvironmentFile = config.sops.secrets.spice_password_env.path;
    };
  };

  sops.secrets.spice_password_env = { };
}
